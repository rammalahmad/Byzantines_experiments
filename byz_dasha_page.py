...
def byz_dasha_page(filename, x_init, A, y, clients_A, clients_y, gamma, num_of_byz, p, mom, num_of_workers, attack, agg,
    ...
    H_w = np.zeros((num_of_workers, len(x)))
    x1 = x.copy()

    for i in range(num_of_workers):
        G_w[i] = logreg_grad(x, [clients_A[i], clients_y[i], l2, setting])
        H_w[i] = logreg_grad(x, [clients_A[i], clients_y[i], l2, setting])
    ...
    for it in range(T):
        ...
        if (bernoulli_arr[bernoulli_counter] == 0):
        ...
        for i in range(num_of_workers - num_of_byz):
            ...
            y_i = clients_y[i]
            if (bernoulli_arr[bernoulli_counter] == 1):
                new_h = logreg_grad(deepcopy(x), [A_i, y_i, l2, setting])
            else: 
                new_h = H_w[i] + logreg_grad(deepcopy(x), [A_i[indices_arr[i]], y_i[indices_arr[i]], l2, setting]) - logreg_grad(x1, [A_i[indices_arr[i]],
                                  y_i[indices_arr[i]], l2, setting])
            G_w[i] += sparsificator(new_h - H_w[i] -mom*(G_w[i]-H_w[i]), sparsificator_params)[0]
            H_w[i] = new_h
    ...
    with open("dump/" + filename + "_Byz_DASHA_PAGE_" + attack + "_" + agg + "_gamma_" + str(gamma) + "_l2_" + str(l2) + "_p_" + str(p) + "_epochs_" + str(T) + "_workers_" + str(num_of_workers) + "_batch_" + str(batch_size) + "_byz_" + str(num_of_byz)+ ".txt", 'wb') as file:

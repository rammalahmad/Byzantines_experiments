...
def byz_vr_marina(filename, x_init, A, y, clients_A, clients_y, gamma, num_of_byz, p, num_of_workers, attack, agg,
    ...
    for it in range(T):
        ...
        for i in range(num_of_workers - num_of_byz):
            ...
            y_i = clients_y[i]
            if (bernoulli_arr[bernoulli_counter] == 1):
                g_i = logreg_grad(deepcopy(x), [A_i, y_i, l2, setting])
            else: 
                ...
                g_i = G_w1 + sparsificator(grad_diff, sparsificator_params)[0]
            G_w[i] = g_i
    ...
    with open("dump/" + filename + "_Byz_VR_MARINA_" + attack + "_" + agg + "_gamma_" + str(gamma) + "_l2_" + str(l2) + "_p_" + str(p) + "_epochs_" + str(T) + "_workers_" + str(num_of_workers) + "_batch_" + str(batch_size) + "_byz_" + str(num_of_byz) + ".txt", 'wb') as file:
